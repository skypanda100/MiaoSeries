...
class FinanceChart : public MultiChart
{
    ...
    //zhengdongtian MACD
    public: XYChart* addMACD(int height, int period1, int period2, int period3, int color,
        int signalColor, int divColor1, int divColor2)
    {
        XYChart *c = addIndicator(height);
        char buffer[1024];
        ...
        //Divergence
        DoubleArray macdArray = (DoubleArray)(ArrayMath(macd).sub(macdSignal));
        double *plus_p = new double[macdArray.len];
        double *minus_p = new double[macdArray.len];
        for(int i = 0;i < macdArray.len;i++){
            plus_p[i] = macdArray[i] > 0 ? macdArray[i] : 0;
            minus_p[i] = macdArray[i] <= 0 ? macdArray[i] : 0;
        }
        addBarIndicator2(c, DoubleArray(plus_p, macdArray.len), divColor1, "Divergence");
        addBarIndicator2(c, DoubleArray(minus_p, macdArray.len), divColor2, "Divergence");
    ...
    //zhengdongtia RSI
    public: XYChart* addRSI(int height, int period1, int period2, int period3, int color1, int color2, int color3, double range, int upColor,
        int downColor)
    {
        XYChart *c = addIndicator(height);
        char buffer[1024];
        sprintf(buffer, "RSI (%d)", period1);
        LineLayer *layer1 = addLineIndicator2(c, computeRSI(period1), color1, buffer);

        sprintf(buffer, "RSI (%d)", period2);
        addLineIndicator2(c, computeRSI(period2), color2, buffer);

        sprintf(buffer, "RSI (%d)", period3);
        addLineIndicator2(c, computeRSI(period3), color3, buffer);

        //Add range if given
        if ((range > 0) && (range < 50)) {
            addThreshold(c, layer1, 50 + range, upColor, 50 - range, downColor);
        }
        c->yAxis()->setLinearScale(0, 100);
        return c;
    }
    ...
    //zhengdongtian KDJ
    public: XYChart* addKDJ(int height, int period1, int period2, int period3, int color1, int color2, int color3)
    {
        ArrayMath movLow = ArrayMath(m_lowData).movMin(period1);
        ArrayMath movRange = ArrayMath(m_highData).movMax(period1).sub(movLow);
        ArrayMath stochastic = ArrayMath(m_closeData).sub(movLow).financeDiv(movRange, 0.5
        ...
        sprintf(buffer, "%%D (%d)", period2);
        addLineIndicator2(c, stochastic.movAvg(period2), color2, buffer);
        sprintf(buffer, "%%J (%d)", period3);
        DoubleArray kArray = (DoubleArray)stochastic;
        DoubleArray dArray = (DoubleArray)stochastic.movAvg(period2);
        double *j_p = new double[kArray.len];
        for(int i = 0;i < kArray.len;i++){
            double kval = kArray[i];
            double dval = dArray[i];
            double jval = 3 * kval - 2 * dval;
            j_p[i] = jval;
        }
        addLineIndicator2(c, DoubleArray(j_p, kArray.len), color3, buffer);
        delete j_p;
